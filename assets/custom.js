  ...
  function searchElement() {
    if ($(".recommendation-modal__container").length > 0) {
      ...
      $(".recommendation-modal__container")
        .find(".recommendation-modal__benefits")
        .find("ul")
        .find("li")
        .each(function (index) {
          if (index == 1) {
            ...
            $(".recommendation-modal__container")
              ...
              .find("ul")
              .append(
                `<li style="color: rgb(51, 51, 51)">We ship TAX free to ${countryName}</li>`
              );
          }
        });
    }
    ...
    if ($(".shopify-payment-button__button").length > 0) {
      if (
        $(".shopify-payment-button__button").text().trim().toLowerCase() ==
        "buy it now"
      ) {
...
function displayPreorderMessage() {
  if (window.location.pathname.indexOf("/products/" > -1)) {
    var variant_id = window.location.search.replace("?variant=", "");
    $(".order_soon_div").removeClass("active");
    $(`[data-variant-id="${variant_id}"]`).addClass("active");
  }
}

$(document).on("click", ".tt-options-swatch li a", function () {
